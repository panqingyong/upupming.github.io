---
title: KaTeX Test with hexo-theme-melody
date: 2018-10-18 20:36:00
tags:
- melody
- hexo
categories:
...
Change your `melody.yml` to load $\KaTeX$ css:
...
To use $\KaTeX$ in melody, you need to uninstall your old hexo render, and then install `hexo-renderer-markdown-it-plus`:
...
|$\gdef\bar#1{% raw %}{#{% endraw %}1^2} \bar{y} + \bar{y}$ | `\gdef\bar#1{% raw %}{#{% endraw %}1^2} \bar{y} + \bar{y}`
...
$\textcolor{% raw %}{#{% endraw %}228B22}{F=ma}$ `\textcolor{% raw %}{#{% endraw %}228B22}{F=ma}`<br>
